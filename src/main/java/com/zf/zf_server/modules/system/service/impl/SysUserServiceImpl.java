...
public class SysUserServiceImpl extends ServiceImpl<SysUserDao, SysUser> implements SysUserService {
    ...
    public R<?> changePassword(String oldPassword, String newPassword, String token) {
        // 获取登录用户信息
        R<SysUser> sysUser = getUser(token);
        if (!sysUser.ok()) {
            return sysUser;
        }
    ...
    public R<?> saveWxOpenId(String token, String openid) {
        ...
        sysUser.getData().setWxOpenid(openid);
        //更新openId
        int resCount = this.baseMapper.updateById(sysUser.getData());
        return resCount > 0 ? R.ok(sysUser) : R.failed(ResponseInfoConstant.OPERATE_FAIL);
    }

    @Override
    public R<SysUser> getUserInfo(String token) {
        R<SysUser> res = getUser(token);
        if (res.ok()) {
            res.getData().setPassword("");
        }
        return res;
    }
    ...
    public R<?> checkWxAuth(String token) {
        ...
        if (!StrUtil.isEmpty(res.getData().getWxOpenid())) {
            return R.ok("微信已经注册");
        }
        return R.failed(ResponseInfoConstant.NO_WX_OPENID);
    }

    private R<SysUser> getUser(String token) {
        // 获取登录用户信息
        R<SysUser> sysUser = sysLoginInfoService.getUser(token);
        if (sysUser.ok()) {
            return sysUser;
